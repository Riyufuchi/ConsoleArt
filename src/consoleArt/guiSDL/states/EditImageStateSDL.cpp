...
EditImageStateSDL::EditImageStateSDL(sdl::WindowInfo& winInfo, ButtonBuilder& buttons, Controller& controller, StateManager& stateManager) : StateSDL(winInfo), AbstractState(controller, stateManager), buttons(buttons)
{
	...
	this->pane->addComponent(0, new sdl::ImageButtonSDL(200, 100, buttons.getButtonTextureFor(ButtonType::CONVER_TO_ASCII, false), [&]() { stateManager.switchState(WindowState::ASCII_CONVERTER); }));
	this->pane->addComponent(0, new sdl::ImageButtonSDL(100, 100, buttons.getButtonTextureFor(ButtonType::SHOW_IMAGE, true), [&]() { stateManager.switchState(WindowState::SHOW_IMAGE); }));
	this->pane->addComponent(1, new sdl::ImageButtonSDL(200, 100, buttons.getButtonTextureFor(ButtonType::IMAGE_FILTER, false),
			[&]() { std::thread([&](){ controller.getMessenger().messageUser(AbstractNotifier::WARNING, "Not yet implemented."); }).detach(); }));
	this->pane->addComponent(1, new sdl::ImageButtonSDL(100, 100, buttons.getButtonTextureFor(ButtonType::IMAGE_INFO, true), [&]()
	{
		if (controller.getSelectedImage())
			controller.getMessenger().displayImageInfo(*controller.getSelectedImage());
	}));
	this->pane->addComponent(2, new sdl::ImageButtonSDL(200, 100, buttons.getButtonTextureFor(ButtonType::WATERMARK, false), [&]() { stateManager.switchState(WindowState::WATERMARK); }));
	this->pane->addComponent(2, new sdl::ImageButtonSDL(100, 100, buttons.getButtonTextureFor(ButtonType::BACK, true), [&]() { stateManager.switchState(WindowState::MAIN); }));
